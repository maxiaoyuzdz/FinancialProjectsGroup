...
int GetClosestMAcroessType()
{
  ...
  for( int i = 0; i < 2000; i++ )
  {
    ...
    double ema_5_0 = iMA( NULL, 0, 5, 0, MODE_EMA, PRICE_CLOSE, i );
    double ema_15_0 = iMA( NULL, 0, 15, 0, MODE_EMA, PRICE_CLOSE, i );

    double ema_5_1 = iMA( NULL, 0, 5, 0, MODE_EMA, PRICE_CLOSE, j );
    double ema_15_1 = iMA( NULL, 0, 15, 0, MODE_EMA, PRICE_CLOSE, j );
    //gold
    if( ema_5_0 >= ema_15_0 && ema_5_1 < ema_15_1 )
    {
      return 1;
    }
...
//|  function                                                   |
//+------------------------------------------------------------------+
void AssessOrderCloseForTP()
{

}

void AssessOrderCloseForSL()
{
...
// has new bar process
  ...
  if( hasnewbar )
  {
    ...
    double ema15_0_close = iMA( NULL, 0, 15, 0, MODE_EMA, PRICE_CLOSE, 0 );
    ...
    double ema15_1_close = iMA( NULL, 0, 15, 0, MODE_EMA, PRICE_CLOSE, 1 );
    ...
    */
    ...
    if( main_sell_conn && !hasorderres )
    {
      if( Open[1] > ema5_1_high && Close[1] > ema5_1_high && Close[1] > Open[1] && ema15_0_close < ema5_0_low )
      {
    ...
    if( main_buy_conn && !hasorderres )
    {
      if( Open[1] < ema5_1_low && Close[1] < ema5_1_low && Close[1] < Open[1] && ema15_0_close > ema5_0_high )
      {
    ...
    if( main_sell_conn && !hasorderres )
    {
      if( Open[1] < ema5_1_close && Close[1] > ema5_1_high && ema15_0_close < ema5_0_low )
      {
    ...
    if( main_buy_conn && !hasorderres )
    {
      if( Open[1] > ema5_1_close && Close[1] < ema5_1_low && ema15_0_close > ema5_0_high )
      {
    ...
    if( hasorderres && generate_ordertype != 0 )
    {
      if( generate_ordertype == 1 )
      {
        ...
        else
        {
          ...
          Alert("send buy order error code = ", errorcode);
        }
      }
      ...
      if( generate_ordertype == -1 )
      {
        ...
        {
          Alert("send sell order error code = ", errorcode);
        }
      }
...
void AnalyzeClose()
{
  ...
  if( res )
  {
    ...
    Alert("close successful, ticket = ", order_ticket);
  }
  else
  {
    Alert("close order error code = ", errorcode);
  }
