...
// Enter the market if there is no command to exit the market
   ...
   AskCCI();
    ...
    if(Decision==DECISION_BUY && NotBuy==false){
      ...
      MAFastCurrent=iMA(NULL,0,3,0,MODE_SMMA,PRICE_MEDIAN,0);
      MAFastPrev=iMA(NULL,0,3,0,MODE_SMMA,PRICE_MEDIAN,1);
      MASlow=iMA(NULL,0,10,2,MODE_SMMA,PRICE_MEDIAN,0);
      ...
      MomentCurrent = iMomentum(NULL,0,5,PRICE_CLOSE,0);
      MomentPrevious = iMomentum(NULL,0,5,PRICE_CLOSE,1);
...
int TradeSell(){
    ...
    ticket = OrderSend(Symbol(),OP_SELL,Lots*mMultiply,Bid,3,0,0,"TurtleTrader(SELL)",0,3,Red);
    if(ticket > 0){
		if(OrderSelect(ticket,SELECT_BY_TICKET,MODE_TRADES)) {
			OrderModify(OrderTicket(), OrderOpenPrice(), StopLossValue, TakeProfitValue, 3, Green);
		}
...
int AskStochastic()
{
   CHOCurrent = iCustom(NULL,0,"CHO",5,3,0,1,0);
   CHOPrevious = iCustom(NULL,0,"CHO",5,3,0,1,1);
...
{
      // CCI VALUE 
   CCICurrent = iCCI(NULL,0,10,PRICE_TYPICAL,0);
   CCIPrevious = iCCI(NULL,0,10,PRICE_TYPICAL,1);
//   CCI_Status   =  DECISION_UNKNOWN;
...
{
      // RSI VALUE 
   RSICurrent = iRSI(NULL,0,6,PRICE_CLOSE,0);
   RSIPrevious = iRSI(NULL,0,6,PRICE_CLOSE,1);
   RSI_Status   =  DECISION_UNKNOWN;
...
int ExitMarket ()
  {
   ...
   CHOCurrentExit = iCustom(NULL,PERIOD_M1,"CHO",5,3,0,1,0);
   CHOPreviousExit = CHOPrevious;
   //CHOPreviousExit = iCustom(NULL,PERIOD_M1,"CHO",5,3,0,1,1);
   RSICurrentExit = iRSI(NULL,PERIOD_M1,6,PRICE_CLOSE,0);
...
int AskMACD()
  {
   ...
   for(int i = 0 ; i < MACDLevel ; i ++)
     {
         P_Time=MathPow(2,i);
       if(iMACD(Symbol(), P_Time, 15,35, 9, PRICE_CLOSE, MODE_SIGNAL, 0) < 
          iMACD(Symbol(), P_Time, 15,35, 9, PRICE_CLOSE, MODE_MAIN, 0))
         {
           SellIndex += iMACD(Symbol(), P_Time, 15,35, 9, PRICE_CLOSE, MODE_MAIN,0);
         }
       if(iMACD(Symbol(), P_Time, 15,35, 9, PRICE_CLOSE,MODE_SIGNAL, 0) > 
          iMACD(Symbol(), P_Time, 15,35, 9, PRICE_CLOSE,MODE_MAIN ,0)  )
         {
           BuyIndex += iMACD(Symbol(), P_Time, 15,35, 9, PRICE_CLOSE, MODE_MAIN, 0);
         }
